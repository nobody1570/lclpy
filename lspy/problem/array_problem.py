from lspy.problem.abstract_local_search_problem \
    import AbstractLocalSearchProblem
import numpy
from lspy.aidfunc.error_func import not_multi_move_type
...
class ArrayProblem(AbstractLocalSearchProblem):
    ...
    .. doctest::
        ...
        >>> from lspy.evaluation.tsp_evaluation_function \\
    ...
    def get_moves(self):
        ...
        .. doctest::
            ...
            >>> import numpy
            >>> from lspy.localsearch.move.tsp_array_swap \\
            ...     import TspArraySwap
            >>> from lspy.evaluation.tsp_evaluation_function \\
            ...     import TspEvaluationFunction
            >>> from lspy.problem.array_problem import ArrayProblem
    ...
    def diff_state(self, old_state):
        ...
        .. doctest::
            ...
            >>> problem.state()
            (0, 1, 2, 3)
            >>> problem.diff_state((0, 1, 2, 3))
            0
            >>> problem.diff_state((0, 2, 1, 3))
            1.5
            >>> problem.diff_state((0, 2, 3, 1))
            2
            >>> problem.diff_state((3, 1, 2, 0))
            1.5
            >>> # with other order
            >>> problem.move((0, 3))
            >>> problem.move((1, 2))
            >>> problem.state()
            (3, 2, 1, 0)
            >>> problem.diff_state((3, 2, 1, 0))
            0
            >>> problem.diff_state((0, 2, 1, 3))
            1.5
            >>> problem.diff_state((3, 2, 0, 1))
            2.5
            >>> problem.diff_state((3, 1, 2, 0))
            1.5
